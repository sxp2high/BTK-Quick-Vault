...
[] spawn {
	...
	// Add keyhandler
	_keyHandlerDown = (findDisplay 46) displayAddEventHandler ["KeyDown", "if (((_this select 1) in (actionKeys 'GetOver')) && (isNil {player getVariable 'btk_quickvault_busy'}) && (alive player) && ((vehicle player) isKindOf 'Man') && ((speed player) > 10) && ((currentWeapon player) == (primaryWeapon player)) && (primaryWeapon player != '') && ((getFatigue player) < 0.6)) then { [player] call BTK_quickvault_fnc_vault; };"];
